#!/usr/bin/env python3
"""
split_hamiltonian_flow_rt_conditional_shape.py

Hamiltonian/divergence-free triangular split RT flow matching for SPD matrices.
The scalar volume flow is trained marginally, and the Hamiltonian shape flow is
conditioned on the (final, fixed) standardized log-volume. This implements
    q(v, z) = q(v) q(z | v)
while preserving divergence-freeness in z for every fixed v.

The command-line interface is unchanged from
split_hamiltonian_flow_rt_shared.py, so run_wishart_spd_baselines_all4_v8.py
can call this file without modification.
...
The shape velocity is u(z,t)=J grad_z H(z,t) with a constant skew-symmetric J.
For odd shape dimension, this script uses a zero one-dimensional null block;
this keeps the field exactly divergence-free, but the null coordinate has zero
Hamiltonian velocity.

Example:
  python split_hamiltonian_flow_rt_shared.py \
    --data-npz results_wishart/wishart_p10_mix_data.npz \
    --out-npz results_wishart/wishart_p10_mix_split_hamiltonian.npz
"""
...
def skew_J(dim: int, device, dtype) -> torch.Tensor:
    """Constant skew-symmetric matrix with 2x2 symplectic blocks and optional null block."""
    J = torch.zeros(dim, dim, device=device, dtype=dtype)
    m = dim // 2
    for k in range(m):
        i = 2 * k
        J[i, i + 1] = 1.0
...
class DivFreeShapeFlow(nn.Module):
    def __init__(self, dim: int, hidden=128, device=None, dtype=torch.float32):
        super().__init__()
        self.dim = dim
        self.net = nn.Sequential(
        ...
        self.register_buffer("J", skew_J(dim, device=device, dtype=dtype))
    ...
    def forward(
        ...
        H = self.hamiltonian(z_req, v_condition, t).sum()
        grad = torch.autograd.grad(H, z_req, create_graph=True)[0]
        return grad @ self.J.T
...
def sample_shape(
    ...
    v_condition = v_condition.detach()
    dt = 1.0 / steps
    ...
    for _ in range(steps):
        ...
        k1 = model(z1, v_condition, t0).detach()
        z2 = (z + 0.5 * dt * k1).detach().requires_grad_(True)
        k2 = model(z2, v_condition, t0 + 0.5 * dt).detach()
        z3 = (z + 0.5 * dt * k2).detach().requires_grad_(True)
        k3 = model(z3, v_condition, t0 + 0.5 * dt).detach()
        z4 = (z + dt * k3).detach().requires_grad_(True)
        k4 = model(z4, v_condition, t0 + dt).detach()
...
def main():
    ...
    if shape_dim % 2 == 1:
        print(f"WARNING: shape_dim={shape_dim} is odd; using a skew J with one null coordinate.")
    ...
    shape_model = DivFreeShapeFlow(shape_dim, hidden=args.hidden_z, device=device, dtype=dtype).to(device)
    opt_v = optim.Adam(vol_model.parameters(), lr=args.lr_v)
    opt_z = optim.Adam(shape_model.parameters(), lr=args.lr_z)

    t0 = time.time()
    print("\nTraining RT Hamiltonian volume model...")
    ...
    print("\nTraining RT Hamiltonian conditional divergence-free shape model...")
    ...
    diagnostics["method"] = "RT Hamiltonian conditional shape|volume"
    diagnostics["train_seconds"] = float(train_seconds)
    diagnostics["sample_seconds"] = float(sample_seconds)
    print_diagnostics("RT Hamiltonian conditional diagnostics", diagnostics)
